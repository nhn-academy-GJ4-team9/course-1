public class Main_gichang01 {
    ...
    public static String solution(String my, int[][] queries) {
        StringBuilder answer = new StringBuilder(my);

        for (int i = 0; i < queries.length; i++) {
            StringBuilder tmp = new StringBuilder(answer.substring(queries[i][0], queries[i][1] + 1)).reverse();
            answer.replace(queries[i][0], queries[i][1] + 1, tmp.toString());
        }
